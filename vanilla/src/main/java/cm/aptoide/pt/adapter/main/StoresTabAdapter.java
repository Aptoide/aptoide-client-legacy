...
import cm.aptoide.pt.viewholders.main.AddStoreViewHolder;
...
public class StoresTabAdapter extends RecyclerView.Adapter<BaseViewHolder> implements SpannableRecyclerAdapter {
    ...
    public BaseViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        switch (viewType){
            ...
            case R.layout.layout_header: //unused for now
                holder = new HeaderViewHolder(LayoutInflater.from(parent.getContext()).inflate(viewType, parent, false), viewType, EnumStoreTheme.APTOIDE_STORE_THEME_DEFAULT);
                break;
            ...
            case R.layout.row_store_item:
                ...
                if (storeItem.id == -1 || TextUtils.isEmpty(storeItem.storeAvatar)) {
                    Glide.with(context).fromResource().load(R.drawable.ic_avatar_apps)
                            .transform(new CircleTransform(context)).into(holder.storeAvatar);
                } else {
                    Glide.with(context).load(storeItem.storeAvatar)
                            .transform(new CircleTransform(context)).into(holder.storeAvatar);
                }
                ...
                holder.storeUnsubscribe.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(final View view) {
                        AptoideDialog.msgBoxYesNo(view.getContext(), storeItem.storeName, Aptoide.getContext().getString(R.string.unsubscribe_yes_no), new DialogInterface.OnClickListener() {
                            @Override
                            public void onClick(DialogInterface dialogInterface, int i) {

                                if (i == AptoideDialog.MSG_BUTTON_YES) {
                                    removeStore(storeItem.id);
                                    Toast.makeText(view.getContext(), Aptoide.getContext().getString(R.string.unsubscribed_) + storeItem.storeName, Toast.LENGTH_SHORT).show();
                                }
                            }
                        }, true);
                    }
