...
public class ABTestingManager {
    ...
    public static final String APP_VIEW_BUTTON_FONT_COLOR_VARIABLE = "AppViewButtonFontColor";
    public static final String APP_VIEW_BUTTON_BACKGROUND_COLOR_VARIABLE = "AppViewButtonBackgroundColor";

    private static Var<String> appViewButtonBackgroundColor = Var.define(APP_VIEW_BUTTON_BACKGROUND_COLOR_VARIABLE, "#E17117");
    private static LiveVariable<String> appViewButtonFontColor = Optimizely.stringForKey(APP_VIEW_BUTTON_FONT_COLOR_VARIABLE, "#FFFFFF");
    ...
    /**
     * Starts a new session for AB testing. When this is called is important it will help to define what is the session length
     * important for analytics metrics. Should be called from launcher {@link android.app.Activity}.
     *
    ...
    /**
     * Adds AB testing resources to current application {@link Resources}. The result of this method should be returned
     * on {@link Application#getResources()}.
     *
     ...
     * @return result of parsing the variable using {@link Color#parseColor(String)} or black if string could not be parsed.
     */
    public static int getColorVariable(String key) {
        switch (key) {
            case APP_VIEW_BUTTON_FONT_COLOR_VARIABLE:
                return parseColorOrDefault(appViewButtonFontColor.get(), Color.BLACK);
            case APP_VIEW_BUTTON_BACKGROUND_COLOR_VARIABLE:
                return parseColorOrDefault(appViewButtonBackgroundColor.stringValue(), Color.BLACK);
            default:
                throw new IllegalArgumentException("Variable not defined for AB Testing: " + key);
        }
    }
    ...
    private static int parseColorOrDefault(String colorString, int defaultColorInt) {
        try {
            ...
            Logger.e("AB-TESTING", "Could not parse AB Testing color: " + colorString + "return default color " + defaultColorInt);
