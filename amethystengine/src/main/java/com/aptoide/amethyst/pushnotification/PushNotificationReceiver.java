...
public class PushNotificationReceiver extends BroadcastReceiver {
  ...
  public static final long PUSH_NOTIFICATION_TIME_INTERVAL = AlarmManager.INTERVAL_HALF_HOUR;
  ...
  public final String PUSH_NOTIFICATION_Action_FIRST_TIME =
      Aptoide.getConfiguration().getActionFirstTime();
  ...
  public interface Notifications {
    ...
    @POST("/3/getPushNotifications") @FormUrlEncoded PushNotificationJson getPushNotifications(
        @FieldMap HashMap<String, String> parameters);
  }

  public static void createPendingIntent(AlarmManager am, Context context, String action, long time,
      int broadcastid) {
    Intent i = new Intent(context, Aptoide.getConfiguration().getPushNotificationsReceiver());
    i.setAction(action);
    PendingIntent pi =
        PendingIntent.getBroadcast(context, broadcastid, i, PendingIntent.FLAG_UPDATE_CURRENT);
    ...
    if (action != null) {
      ...
      if (action.equals(Intent.ACTION_BOOT_COMPLETED)) {
        ...
        Executors.newSingleThreadExecutor().execute(new Runnable() {
          @Override public void run() {
            try {
              ...
              int lastId = PreferenceManager.getDefaultSharedPreferences(context)
              ...
              PushNotificationJson response =
                  new RestAdapter.Builder().setConverter(OauthErrorHandler.createConverter())
                      .setEndpoint("http://webservices.aptoide.com/webservices")
                      .build()
                      .create(Notifications.class)
                      .getPushNotifications(parameters);
                ...
                handler.post(new Runnable() {
                  @Override public void run() {
                    ...
                    try {
                      ...
                      String bannerUrl = notification.getImages() != null ? getSize(
                          notification.getImages().getBanner_url()) : null;

                      ImageLoader.getInstance()
                          .loadImage(bannerUrl, new MyImageLoadingListener(context, extra));
                    } catch (Exception e) {
                      e.printStackTrace();
                    }
                  }
        ...
        try {
          ...
          new OkHttpClient().newCall(new Request.Builder().url(trackUrl).build())
  ...
  public static void setPendingIntents(Context context) {
    ...
    createPendingIntent(am, context, PUSH_NOTIFICATION_Action, PUSH_NOTIFICATION_TIME_INTERVAL,
        PUSH_NOTIFICATION_BROADCAST_ID);
    createPendingIntent(am, context, UPDATE_SERVICE, AlarmManager.INTERVAL_HALF_DAY / 2,
        UPDATE_NOTIFICATION_BROADCAST_ID);

    createPendingIntent(am, context, TIMELINE_POST_SERVICE, AlarmManager.INTERVAL_HALF_DAY / 2,
        TIMELINE_POST_NOTIFICATION_BROADCAST_ID);
    createPendingIntent(am, context, TIMELINE_ACTIVITY_SERVICE, AlarmManager.INTERVAL_HALF_DAY / 2,
        TIMELINE_ACTIVITY_NOTIFICATION_BROADCAST_ID);
  }

  private String getSize(String banner_url) {
    String[] splittedUrl = banner_url.split("\\.(?=[^\\.]+$)");
    banner_url = splittedUrl[0]
        + "_"
        + IconSizeUtils.generateSizeStringNotification()
        + "."
        + splittedUrl[1];
    return banner_url;
  }
  ...
  private void loadNotification(Bundle extra, Context context, Bitmap o) {
    ...
    PendingIntent resultPendingIntent = PendingIntent.getActivity(context,
        PreferenceManager.getDefaultSharedPreferences(context)
            .getInt(SPREF_PNOTIFICATION_ID, new Random().nextInt()), resultIntent, 0);
    ...
    Notification notification =
        new NotificationCompat.Builder(context).setSmallIcon(notificationSmallIcon)
            .setLargeIcon(
                BitmapFactory.decodeResource(context.getResources(), getDrawableResource()))
            .setContentIntent(resultPendingIntent)
            .setAutoCancel(true)
            .setOngoing(false)
            .setContentTitle(extra.getCharSequence(PUSH_NOTIFICATION_TITLE))
            .setContentText(extra.getCharSequence(PUSH_NOTIFICATION_MSG))
            .build();

    if (Build.VERSION.SDK_INT >= 16 && Build.VERSION.SDK_INT < 24 && o != null) {
      /**
